//
//  ReachingforToyOverviewViewController.swift
//  questionApp
//
...
class ReachingforToyOverviewViewController: UIViewController {
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
    // analytics
    Tracker.createEvent(.ReachingForToy, .Load, .Overview)
  }
  ...
  override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
    if segue.identifier == "ReachingForToyEmbeddedVideoSegue" {
      // set the playerVC as the destination
      playerVC = segue.destinationViewController as! AVPlayerViewController
      let path = NSBundle.mainBundle().pathForResource("falling toy", ofType: "mp4")
    ...
    else if segue.identifier == "ReachingForToyWhatWillYouNeedSegueID" {
      playerVC.player.pause()
    }
  }
