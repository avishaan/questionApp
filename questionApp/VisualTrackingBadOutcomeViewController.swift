//
//  VisualTrackingBadOutcomeViewController.swift
//  questionApp
//
...
class VisualTrackingBadOutcomeViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    // analytics
    Tracker.createEvent(.VisualTracking, .Load, .Bad)
  ...
  */
  func initializeViewFromTestHistory() {
    ...
    if failed <= 1 {
      ...
      questionLabel.text = "Didn't track toy?"
      // update infoLabel
      ...
      // update infoLabel
      infoLabel.text = "Practice by slowing down the side-to-side movement.You may also use a toy with lights, but no sounds."
      boldInfoLabel.text = "Try this, then perform the test again in a week."
      ...
      // update infoLabel
      infoLabel.text = "Getting baby's attention from the start is the key."
      boldInfoLabel.text = "If baby is still unable to track the toy and is over a month old, repeat and record test to show your pediatrician."
    } else {
      // update questionLabel
      questionLabel.text = "Didn't track toy?"
  ...
  */
  func scheduleReminder() {
    ...
    if BNLocalNotification.doesLocalNotificationExist(Test.TestNamesPresentable.visualTracking) == false {
      
      // configure the local notification
      let localNotification = BNLocalNotification(nameOfTest: Test.TestNamesPresentable.visualTracking, secondsBeforeDisplayingReminder: Test.NotificationInterval.visualTracking)
