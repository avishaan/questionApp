...
class PointFollowingGoodOutcomeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
      // analytics
      Tracker.createEvent(.PointFollowing, .Load, .Good)

        // If a reminder notification had previously been scheduled, remove it now that the test has been passed.
        BNLocalNotification.removeLocalNotification(Test.TestNamesPresentable.pointFollowing)
    }
    ...
    @IBAction func onFacebookButtonTap(sender: AnyObject) {
        ...
        BNFacebook.postToFacebook(self, testName: Test.TestNamesPresentable.pointFollowing)
    }
}
