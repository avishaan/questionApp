...
class SocialSmilingGoodOutcomeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
      // analytics
      Tracker.createEvent(.SocialSmiling, .Load, .Good)
        
        // If a reminder notification had previously been scheduled, remove it now that the test has been passed.
        BNLocalNotification.removeLocalNotification(Test.TestNamesPresentable.socialSmiling)
    }
    ...
    @IBAction func onFacebookButtonTap(sender: AnyObject) {
        ...
        BNFacebook.postToFacebook(self, testName: Test.TestNamesPresentable.socialSmiling)
    }
}
